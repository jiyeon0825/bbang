// select box
    ...
    // Set the initial value of categoryNo select box
    $('select[name="categoryNo"]').val(selectedCategoryNo);

    // Function to populate the boardNo select box
    ...
    // Event listener for categoryNo select box
    $('select[name="categoryNo"]').on('change', function() {
    populateBoardNo($(this).val());
});

    // Trigger the change event to populate boardNo on page load with selected values
